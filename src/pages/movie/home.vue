<template>
    <view style="height: 100%" v-if="masters">
            ...
            <template v-slot:content>
                ...
                <scroll-view :scroll-top="scrollTop" style="position: fixed;top: 23.5%;z-index: 1024;height: 76.5%" scroll-y="true" @scrolltolower="movieLoadData">
                    ...
                    <view v-if="urlType!='weekly'&&urlType!='us_box'" class="cu-list grid" :class="['col-' + movieGridCol,gridBorder?'':'no-border']">
                        <view class="cu-item" @tap="movieDetail(item.title, item.images.large)" v-for="(item,index) in movieData" :key="index" >
        ...
        <modal :show="modalStatus" @cancel="hideModal" :custom="true">
            <scroll-view class="text-center" style="height: 900rpx" scroll-y="true" >
                <view>{{movieTitle}}</view>
                <image :src="movieImg" mode="widthFix" :class="[false?'cu-avatar':'', false?'round': '']"></image>

                <view class="cu-list menu" :class="[true?'sm-border':'', false?'card-menu margin-top':'']">
                    <view v-if="movieDetailList.length !== 0" class="cu-item" v-for="(keywordList, index) in movieDetailList" :key="index">
                        <view class="content padding-tb-sm">
                            <view>
                                <text class="fa fa-video-camera text-blue margin-right-sm"></text>
                                <text>{{keywordList.name}}</text>
                    ...
                    <view v-if="movieDetailList.length == 0">正在寻找资源...若没有加载请放弃</view>
                </view>
            </scroll-view>
        </modal>
    </view>
...
<script>
    ...
    export default {
        ...
        methods: {
            detail (url) { // 查看电影详情
                uni.navigateTo({
                    url: `/pages/movie/movieList?movieUrl=${encodeURIComponent(JSON.stringify(url))}`
                })
            },
            hideModal () {
                this.modalStatus = false
            },
            async movieDetail (name, img) {
