<template>
    <view class="content">
                ...
                <swiper-item v-for="(item, index) in tabBars" :key="index">
                    <scroll-view style="height: 80%" scroll-y>
                        ...
                        <view class="cu-list menu" :class="[false ? 'sm-border': '', false ? 'card-menu' : '']">
                            <navigator :url="'/pages/webview/webview?url='+item.url+''" v-for="(item, index) in dataLists"
                                       :key="index">
        ...
        <scroll-view scroll-y v-else style="height: 100%">
            <mioSearch class="test" @parentFun="questionListFn" :hotKeywordList="hotKeywordList" style="height: 100%">
                <template v-slot:content>
                    <view class="cu-list menu" v-for="(keywordList, index) in questionBanks" :key="index" :class="[true?'sm-border':'', false?'card-menu margin-top':'']">
                        <view class="cu-item">
                            <view class="content padding-tb-sm">
                                <view>
                                    <text class="fa fa-video-camera text-blue margin-right-sm"></text>
                                    <text>{{keywordList.id}}:{{keywordList.question}}</text>
                                </view>
                            </view>
                            <view v-show="true" class="fa fa-angle-right fa-2x margin-left"></view>
                        </view>
                        <view class="text-center bg-white text-lg padding-xs">
                            <text>anwser：{{keywordList.answer}}</text>
                        </view>
                    </view>
                ...
                <template v-slot:detail>
                    <scroll-view style="height: 85%" scroll-y="true" >
                        <ad v-if="ggkzs" unit-id="adunit-35711c1e83d8593c" ad-type="grid" grid-opacity="0.8" grid-count="5" ad-theme="white"></ad>
                        <view class="cu-list menu" v-for="(keywordList, index) in answerQuestion" :key="index" :class="[true?'sm-border':'', false?'card-menu margin-top':'']">
                            <view class="cu-item">
                                <view class="content padding-tb-sm">
                                    <view>
                                        <text class="fa fa-video-camera text-blue margin-right-sm"></text>
    ...
    export default {
        ...
        methods: {
            questionListFn (val) {
                ...
                setTimeout(() => {
                    this.answerQuestion = this.questionBanks.filter(item => {
        ...
        computed: {
            ...mapState(['software', 'ggkzs', 'masters']),
        },
    }
</script>
